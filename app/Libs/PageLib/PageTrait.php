...
trait PageTrait
{
     ...
     * @return array
     */
    protected  function setErrorAccess(Request $request, $message, $code, $nav_id = null){
        if ($request->ajax()) {
            return [
                'access' => 'failed',
                'message' => $message
            ...
            $this->setForbiddenAccess($code, $nav_id);
        }
    }
     ...
     * @return array
     */
    protected function validateAccess($rule, $roles)
    {
        if (($roles[$rule] == '0') || empty($roles[$rule]) ) {
            // cek request
            return $this->setErrorAccess($this->request, 'maaf, anda tidak  mempunyai akses penuh untuk halaman ini', '403', $this->nav_id);
        }
    }
